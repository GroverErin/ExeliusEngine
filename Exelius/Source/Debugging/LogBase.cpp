...
namespace Exelius
{
	...
	void _Log::Initialize()
	{
		...
		s_pEngineLogger = std::make_shared<spdlog::logger>("EXELIUS", begin(logSinks), end(logSinks));
		...
		s_pClientLogger = std::make_shared<spdlog::logger>("CLIENT", begin(logSinks), end(logSinks));
